...
public final class PoolInfo implements Serializable {
	 ...
	 */
	public PoolInfo(String jcoConn) throws Exception {
		...
		try {
			...
			try {
				...
				throw new Exception("▶▶▶ JCO 연결풀을 생성하지 못함! 에러원인 : " + e.getMessage());
			}
		} catch (Exception e) {
			throw new Exception(e.getMessage());
		}
	...
	protected JCoDestination getConnect() {
		...
		try {
			...
			log.error("[JCO.Exception] error mesg  = " + je.getMessage());
			log.error("[JCO.Exception] error group = " + je.getGroup());
			log.error("[JCO.Exception] error key   = " + je.getKey());
			log.error("[JCO.Exception] error desc  = " + je.toString());
			return clientConn;
		} catch(Exception e) {
