...
public class SAPConnector {
	...
	public SAPConnector(String funcName)  {
		if (funcName==null || funcName.trim().equals(""))
			throw new ApiException("EX01001 RFC 함수이름 오류!");
		...
		try {
			...
			if ( dest==null )
				throw new ApiException("SAP 연결객체가 null!");
		} catch(JCoException je) {
			throw new ApiException("EX01002 " + je.getMessage());
		...
		try {
			...
			throw new ApiException("EX01002 " + ae.getMessage());
		}
	...
	private void createFunction() {
		try {
			...
			throw new ApiException("EX01003" + e.getMessage());
		}
	}
	...
	public JCoParameterList getImportParams(){
		if (function==null)
			throw new ApiException("EX01005 + RFC 함수가 초기화되지 않았습니다.");
	...
	public void execute()  {
		...
		try {
			...
			throw new ApiException("EX01007" + e.getMessage());
		}
		finally {
			timerLog(timer.end());
		}
	...
	public JCoParameterList getExportParams() {
		if (!isExcuted)
			throw new ApiException("EX01008 + RFC 함수가 실행되지 않았습니다.");
